<script>
	...
	const CUSTOM_URL = "<?= base_url('admin/groups'); ?>";
	...
	const loadProductSubCategory = () => {
		fetch(`${BASE_URL}/admin/sub-categories/get/<?= $product_sub_category_id ?>`, {
				method: 'GET'
			})
	...
	const btnEdit = (id) => {
		...
		fetch(`${CUSTOM_URL}/get/${id}`, {
			...
			.then(res => {
				...
				if (res.status) {
					$('#id').val(res.data.id)
					$('#product_sub_category_id').val(res.data.product_sub_category_id)
					$('#group').val(res.data.group)
